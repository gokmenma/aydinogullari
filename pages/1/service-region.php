...
try {
	...
	note =?, 
	creator = ?");
		$ekle->execute(array($title, TODAY . " - " . date("H:i:s") . "", 5, "servicestatus sayfası Servis Durumu statu bilgisidir", sesset("id")));
...
if ($_POST && $_GET["type"] == "update") {
	...
	regdate = ?,
	statu = ?,
	note =?, creator = ? WHERE id = ?", );
	$ekle->execute(array($title, TODAY . " - " . date("H:i:s") . "", 5, "servicestatus sayfası Servis Durumu statu bilgisidir", sesset("id"), $id));
...
<div class="pd-20 bg-white border-radius-16 box-shadow mb-20">
	...
	<form method="POST" id="myForm">
		<div class="clearfix justify-content-between mb-20">
			<div class="pull-left">
				<h5 class="text-blue">Servis Bölgesi Listesi</h5>
			</div>
					...
					<div class="modal-dialog modal-dialog-centered" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLongTitle">Bölge Tanımla</h5>
							...
							<div class="modal-body">
								<div class="form-group">
									...
									<label">
										<font color="red">(*)</font>Servis Bölgesi Bilgisi :
										</label>
	...
	<table class="data-table table-bordered table-hover" style="text-align: center;">
		...
		<tbody>
			<?php
			$cq = $ac->prepare("SELECT * FROM units WHERE statu = '5' ");
			$cq->execute();
			$kx = 1;
...
<script>
	submitButton.on('click', function () {
		addType("service-region", "Servis Bölgesi");
	});
</script>