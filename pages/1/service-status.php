...
try {
	...
	note =?, 
	creator = ?");
		$ekle->execute(array($title, TODAY . " - " . date("H:i:s") . "", 4, "servicestatus sayfası Servis Durumu statu bilgisidir", sesset("id")));
...
if ($_POST && $_GET["type"] == "update") {
	...
	$ekle->execute(array($title, TODAY . " - " . date("H:i:s") . "", 4, "servicestatus sayfası Servis Durumu statu bilgisidir", sesset("id"), $id));
