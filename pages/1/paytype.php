...
if ($_POST && $_GET["type"] == "new") {
	...
	regdate = ?,
	statu = ?");
	$ekle->execute(array($title, TODAY . " - " . date("H:i:s") . "", 3));
...
if ($_POST && $_GET["type"] == "update") {
	...
	regdate = ?,
	statu = ? WHERE id = ?", );
	$ekle->execute(array($title, TODAY . " - " . date("H:i:s") . "", 3, $id));
...
<div class="pd-20 bg-white border-radius-16 box-shadow mb-20">
	...
	<form method="POST" id="myForm">
		<div class="clearfix justify-content-between mb-20">
			<div class="pull-left">
				<h5 class="text-blue">Tahsilat Türü Listesi</h5>
			</div>
					...
					<div class="modal-dialog modal-dialog-centered" role="document">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title" id="exampleModalLongTitle">Tahsilat Türü</h5>
							...
							<div class="modal-body">
								<div class="form-group">
									...
									<label">
										<font color="red">(*)</font>Tahsilat Türü Bilgisi :
										</label>
										<input required id="title" name="title" autocomplete="off"
											placeholder="örn: Peşin" class="form-control" type="text">
								</div>
							</div>
	...
	<table class="data-table table-bordered table-hover" style="text-align: center;">
		...
		<tbody>
			<?php
			$cq = $ac->prepare("SELECT * FROM units WHERE statu = '3' ");
			$cq->execute();
			$kx = 1;
				...
				<tr>
					...
					<td>
						...
						<a class="btn btn-sm btn-danger text-white" data-tooltip="Sil"
							onClick="deleteRecord('Kaydı silmek istediğinize emin misiniz?',<?php echo $as["id"]; ?>,'paytype','units')"><i
