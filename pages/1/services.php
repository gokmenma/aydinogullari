...
$servisler = $Services->getServiceList();
    ...
    <table id="serviceTable" class="data-table table-hover table-bordered table-responsive-sm">
        ...
        <tbody>
            ...
            foreach ($servisler as $servis) {
                $pid = $servis->id;
                $enc_id = Security::encrypt($servis->id);
                $formatted_start_date = Date::dmyHis($servis->pstart_date);
                $sort_start_date = date('Y-m-d H:i:s', strtotime(str_replace('.', '-', $servis->pstart_date)));
                ?>
                <tr>
                    <td class="text-center"><?php echo $pid ?></td>
                    <td>
                        <?php echo $servis->service_number; ?>
                    </td>

                    <td data-tooltip="<?php echo $servis->firma_adi; ?>">

                        <?php echo ($servis->firma_adi); ?>
                    </td>

                    <td class="text-center">
                        <?php echo Helper::getRegionName($servis->region ?? '') ?>
                    </td>


                    <td class="text-center">
                        <?php echo $servis->title ?? ''; ?>
                    </td>
                    <td class="text-center" data-sort="<?php echo (Date::Ymd($servis->pregdate)); ?>">
                        <?php echo (Date::dmyHis($servis->pregdate)); ?>
                    </td>
                    <td class="text-center" data-sort="<?php echo (Date::Ymd($servis->pstart_date)); ?>"> <?php
                       echo (Date::dmyHis($servis->pstart_date));
                       ?>

                    </td>
                    <td class="text-center">
                        <?php
                        echo getSozlesmeStatusBadge($servis->contract_statu);
                        ?>
                    </td>
                    <td class="text-center">
                        <?php
                        echo getStatusBadge($servis->pstatu);
                        ?>
                    </td>
                    <td>
                        <?php echo ($servis->olusturan); ?>
                    </td>
                    ...
                    <td class="text-center" style="width:7%; white-space: nowrap;">
                        ...
                        if (permtrue("serviceDel")) { ?>
                            <button type="button" class="btn btn-sm btn-danger" data-tooltip="Sil"
                                onClick="deleteRecord('<?php echo $servis->id; ?> nolu Servisi silmek istediğinize emin misiniz?','<?php echo $pid; ?>','services','projects')"><i
